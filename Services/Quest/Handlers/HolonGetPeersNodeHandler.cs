...
public sealed class HolonGetPeersNodeHandler : IQuestNodeHandler
{
    ...
    {
        if (!QuestNodeConfig.TryDeserialize<IdConfig>(context.Node.Config, nameof(QuestNodeType.HolonGetPeers), out var cfg, out var cfgError))
            return QuestNodeResults.Fail(cfgError);
        var r = await _holonManager.GetPeersAsync(cfg.Id, context.ActingAvatarId);
