...
These attributes live in `Oasis.SurrealDb.Client.Schema`. POCOs in the
consumer project decorate themselves with them; the `oasis-surreal`
...
## Configuration: OasisSurrealDbOptions

The toolkit reads a single top-level options object,
[OasisSurrealDbOptions](OasisSurrealDbOptions.cs), under the
...
```json
{
  "SurrealDb": {
    ...
    "Generation": {
      ...
      "GeneratedNamespace":    "OASIS.WebAPI.Generated.SurrealDb",
      "EmitSurql":             true,
      "EmitFlowcharts":        true,
      "EmitDbml":              false,
...
```csharp
services.Configure<OasisSurrealDbOptions>(
    configuration.GetSection(OasisSurrealDbOptions.ConfigSectionName));
```
...
[`AttributePocoByteEquivalenceTests`](../../../tests/OASIS.WebAPI.Tests/Persistence/SurrealDb/AttributePocoByteEquivalenceTests.cs)
...
`MermaidParseException`, `AggregateEmitter`, the `Oasis.SurrealDb.SourceGen`
