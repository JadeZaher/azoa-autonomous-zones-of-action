...
// Azoa.SurrealDb.Client -- top-level configuration for the toolkit.
//
// Composes:
...
//
//   services.Configure<AzoaSurrealDbOptions>(Configuration.GetSection("SurrealDb"));
//
// Environment variable mirrors (see AZOA_SURREAL_* in the CLI) override the
...
{
    /// <summary>
    /// Top-level configuration for the Azoa.SurrealDb toolkit. Single entry
    /// point binding both the runtime client (connection + auth) and the
    /// build-time generator (output paths + namespaces).
    /// </summary>
    public sealed class AzoaSurrealDbOptions
    {
        /// <summary>
        /// Configuration-binding key under which an <see cref="AzoaSurrealDbOptions"/>
    ...
    public sealed class ConnectionOptions
    {
        ...
        /// <c>AZOA_SURREAL_URL</c>.
        /// </summary>
        public string Url { get; set; } = "http://localhost:8442";

        /// <summary>Basic-auth user. Env override: <c>AZOA_SURREAL_USER</c>.</summary>
        public string Username { get; set; } = "";

        /// <summary>Basic-auth password. Env override: <c>AZOA_SURREAL_PASS</c>.</summary>
        public string Password { get; set; } = "";

        /// <summary>SurrealDB namespace. Env override: <c>AZOA_SURREAL_NS</c>.</summary>
        public string Namespace { get; set; } = "azoa";

        /// <summary>SurrealDB database. Env override: <c>AZOA_SURREAL_DB</c>.</summary>
        public string Database { get; set; } = "azoa";

        /// <summary>
        /// Optional JWT bearer token. When supplied, this is sent as
        /// <c>Authorization: Bearer ...</c> in preference to basic auth.
        /// Env override: <c>AZOA_SURREAL_JWT</c>.
        /// </summary>
        public string? JwtToken { get; set; }

        /// <summary>
        /// Optional API key passed as <c>X-Api-Key</c>. Used by the
        /// Azoa.WebAPI surface for service-to-service auth. Env override:
        /// <c>AZOA_SURREAL_API_KEY</c>.
        /// </summary>
    ...
    /// <c>azoa-surreal generate</c> / <c>flowcharts</c> CLI subcommands
