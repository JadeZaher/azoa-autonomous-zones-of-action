...
namespace Azoa.SurrealDb.Analyzer;
...
/// <c>Core.SurrealDb.Query</c> or <c>Azoa.SurrealDb.Client.Query</c> is exempt
...
public sealed class SurrealQlSafetyAnalyzerDiagnostic : DiagnosticAnalyzer
{
    ...
    private static readonly DiagnosticDescriptor Rule = new DiagnosticDescriptor(
        ...
        description:
            ...
            "All queries must be composed through Azoa.SurrealDb.Client.Query.SurrealQuery " +
            "with named parameters.",
        helpLinkUri: "https://github.com/azoa/azoa/blob/main/conductor/tracks/surrealdb-migration/spec.md#g3");
    ...
    /// <c>Azoa.SurrealDb.Client.Query.SurrealQuery</c>. Uses the semantic
    /// model (preferred) so that fully-qualified
    /// (<c>Azoa.SurrealDb.Client.Query.SurrealQuery.Of(...)</c>) and aliased
    /// (<c>using SQ = Azoa.SurrealDb.Client.Query.SurrealQuery; SQ.Of(...)</c>)
        ...
        SyntaxNodeAnalysisContext ctx)
    {
        ...
        if (symbol is IMethodSymbol method && method.ContainingType is not null)
        {
            ...
            if (fullName.EndsWith("Azoa.SurrealDb.Client.Query.SurrealQuery", System.StringComparison.Ordinal))
                return true;
    ...
    private static bool IsInsideSafeLayer(SyntaxNode node)
    {
        ...
        //   * Azoa.SurrealDb.Client.Query          — homebake package query layer.
        //   * Azoa.SurrealDb.Schema.Migration      — schema runner (HIGH#6 fallback).
        ...
        return ns.IndexOf("Core.SurrealDb.Query",             System.StringComparison.Ordinal) >= 0 ||
               ns.IndexOf("Azoa.SurrealDb.Client.Query",     System.StringComparison.Ordinal) >= 0 ||
               ns.IndexOf("Azoa.SurrealDb.Schema.Migration", System.StringComparison.Ordinal) >= 0;
    }
