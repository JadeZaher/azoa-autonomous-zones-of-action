...
that calls into the `Azoa.SurrealDb.Client` query layer.
...
`Azoa.SurrealDb.Client.Query` is exempt — these are the safe construction
layers themselves.
...
```xml
<ItemGroup>
  <PackageReference Include="Azoa.SurrealDb.Analyzer" Version="0.1.0" PrivateAssets="all" />
</ItemGroup>
```
...
```xml
<ItemGroup>
  <ProjectReference Include="packages/Azoa.SurrealDb.Analyzer/Azoa.SurrealDb.Analyzer.csproj"
...
`Azoa.SurrealDb.*` package suite is proven internally for 3–6 months.
