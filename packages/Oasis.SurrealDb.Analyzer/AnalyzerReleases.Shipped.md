...
- Analyzer relocated from `analyzers/SurrealQlSafetyAnalyzer/` into the
  companion package `Oasis.SurrealDb.Analyzer`. Namespace changed from
  `OASIS.SurrealQlSafetyAnalyzer` to `Oasis.SurrealDb.Analyzer`.
...
- Allowlist expanded to include the new `Oasis.SurrealDb.Client.Query`
