...
public class AlgorandProviderCustodySigningTests : IDisposable
{
    ...
    private IConfiguration BuildConfig() => new ConfigurationBuilder()
        ...
        {
            ["OASIS:WalletEncryptionKey"] = "unit-test-wallet-encryption-key-0123456789",
            ...
            ["OASIS:Algorand:PlatformMnemonic"] = new AlgoAccount().ToMnemonic(),
    ...
    public async Task PerUserTransfer_SignsWithUsersKey_ViaCustodyResolver_NotPlatform()
    {
        ...
        custody.Setup(c => c.WithSigningKeyAsync(
                It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<Func<byte[], Task<OASISResult<byte[]>>>>()))
            .Returns(async (Guid wId, Guid aId, Func<byte[], Task<OASISResult<byte[]>>> sign) =>
            {
                resolvedWallet = wId;
                resolvedAvatar = aId;
                ...
                return new OASISResult<OASISResult<byte[]>> { Result = inner };
            });
        // The platform door must NOT be taken for a per-user op.
        custody.Setup(c => c.WithPlatformSigningKeyAsync(
                It.IsAny<bool>(), It.IsAny<Func<byte[], Task<OASISResult<byte[]>>>>()))
        ...
        custody.Verify(c => c.WithSigningKeyAsync(walletId, avatarId,
            It.IsAny<Func<byte[], Task<OASISResult<byte[]>>>>()), Times.Once);
        custody.Verify(c => c.WithPlatformSigningKeyAsync(
            It.IsAny<bool>(), It.IsAny<Func<byte[], Task<OASISResult<byte[]>>>>()), Times.Never);
    }
    ...
    public async Task PerUserTransfer_NonOwningAvatar_IsIdorRejected_WithNoSigningSideEffect()
    {
        ...
        custody.Setup(c => c.WithSigningKeyAsync(
                It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<Func<byte[], Task<OASISResult<byte[]>>>>()))
            .Returns((Guid wId, Guid aId, Func<byte[], Task<OASISResult<byte[]>>> sign) =>
                // sign is never invoked under an IDOR rejection (guard returns first).
                Task.FromResult(new OASISResult<OASISResult<byte[]>>
                {
                    IsError = true,
    ...
    public async Task PerUserTransfer_UnresolvableContext_FailsClosed_NeverPlatformFallback()
    {
        ...
        var provider = new AlgorandProvider(
            config, NullLogger<AlgorandProvider>.Instance, signerFactory,
            keyService: new OASIS.WebAPI.Core.WalletKeyService(config),
