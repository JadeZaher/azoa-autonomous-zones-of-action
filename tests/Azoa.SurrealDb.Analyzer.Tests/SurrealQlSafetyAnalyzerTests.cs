...
namespace Azoa.SurrealDb.Analyzer.Tests;
...
public sealed class SurrealQlSafetyAnalyzerTests
{
    ...
    // Stub lives in the new Azoa.SurrealDb.Client.Query allowlist namespace; that
    ...
    {
        // Code in the Azoa.SurrealDb.Client.Query namespace must NOT be flagged
    ...
    {
        // The legacy AZOA.WebAPI Core.SurrealDb.Query namespace must remain
        // allowlisted while the AZOA.WebAPI ProjectReference still points at
        // the legacy types (A5 deletes it).
        const string source = """
            namespace AZOA.WebAPI.Core.SurrealDb.Query
            {
                public sealed class SurrealQuery
                {
                    public static SurrealQuery Of(string sql) => new SurrealQuery();
    ...
    {
        // Note: NO `using Azoa.SurrealDb.Client.Query;` — the call is fully-
        // qualified. The pre-fix analyzer compared the receiver token literally
        // ("global::Azoa.SurrealDb.Client.Query.SurrealQuery" != "SurrealQuery")
        ...
        const string source = """
            class TestClass
            {
                void Test(string userInput)
                {
                    ...
                    var q = global::Azoa.SurrealDb.Client.Query.SurrealQuery.Of({|SRDB0001:sql|});
                }
            }
            """;
    ...
    public async Task Fail_aliased_SurrealQuery_Of_with_interpolated_initializer()
    {
        ...
        const string source = """
            using SQ = Azoa.SurrealDb.Client.Query.SurrealQuery;
    ...
    public async Task Pass_fully_qualified_SurrealQuery_Of_with_literal_is_not_flagged()
    {
        ...
        const string source = """
            class TestClass
            {
                void Test()
                {
                    var q = global::Azoa.SurrealDb.Client.Query.SurrealQuery
