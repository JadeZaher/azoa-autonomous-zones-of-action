...
// Oasis.SurrealDb.Schema.Tests -- Live migration round-trip.
//
...
// using a dedicated namespace ("oasis_migration_test") and database
...
namespace Oasis.SurrealDb.Schema.Tests.Migration
{
    ...
    public class MigrationRunnerLiveTests
    {
        ...
        private const string TestNamespace = "oasis_migration_test";
        ...
        public async Task Up_applies_a_data_migration_after_the_schema_phase()
        {
            ...
            var migrationsDir = Path.Combine(Path.GetTempPath(),
                "oasis_migration_test_" + Guid.NewGuid().ToString("N"));
