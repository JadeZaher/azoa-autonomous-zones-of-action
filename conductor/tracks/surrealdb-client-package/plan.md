...
### Phase 1 — Bootstrap
1. [ ] Create `packages/Azoa.SurrealDb.Client/` + `.Schema/` + `.Analyzer/` directories with `Azoa.SurrealDb.<Name>.csproj` (netstandard2.0 + net8.0 multi-target; version 0.1.0; package metadata: authors, description, repo URL; `IsPackable=true`; `IsPublishable=false` until decision)
2. [ ] Create matching `tests/Azoa.SurrealDb.<Name>.Tests/` projects (xunit + FluentAssertions + Moq, matching versions in main test project)
3. [ ] Wire all packages + test projects into `azoa.sln`
4. [ ] Add `Directory.Build.props` at repo root with `<AzoaSurrealDbVersion>0.1.0</AzoaSurrealDbVersion>` MSBuild property; future consumers reference via `Version="$(AzoaSurrealDbVersion)"`
...
11. [ ] Move `Core/SurrealDb/Query/SurrealQuery.cs`, `SurrealIdentifier.cs`, `ISurrealExecutor.cs`, `SurrealExecutor.cs` from `AZOA.WebAPI` INTO `packages/Azoa.SurrealDb.Client/Query/`. Keep public API unchanged for now; deletions in AZOA happen in Phase 6
...
24. [ ] CLI tool `azoa-surreal` (`<PackAsTool>true</PackAsTool>` on `Azoa.SurrealDb.Schema.csproj`): `azoa-surreal migrate up`, `migrate status`, `migrate dry-run`, `generate <file.mermaid>`, `validate <file>`. Reads connection config from env vars or `--connection` flag
...
26. [ ] Move `analyzers/SurrealQlSafetyAnalyzer/` contents INTO `packages/Azoa.SurrealDb.Analyzer/`. Update target framework to netstandard2.0 (analyzers must). Update `AnalyzerReleases.Shipped.md` to mark v0.1.0
27. [ ] Companion-package wiring: `Azoa.SurrealDb.Analyzer.csproj` declares analyzer assets; consumers add `PackageReference Include="Azoa.SurrealDb.Analyzer" PrivateAssets="all"` (so the analyzer doesn't transit through to their consumers)
...
### Phase 6 — AZOA integration (delete + replace)
30. [ ] Add `ProjectReference` from `AZOA.WebAPI.csproj` to `packages/Azoa.SurrealDb.Client/Azoa.SurrealDb.Client.csproj` and `packages/Azoa.SurrealDb.Analyzer/...csproj` (the latter with `OutputItemType="Analyzer" ReferenceOutputAssembly="false"`)
31. [ ] Remove `<PackageReference Include="SurrealDb.Net" Version="[0.10.2]" />` from `AZOA.WebAPI.csproj` and from `tests/AZOA.WebAPI.IntegrationTests/AZOA.WebAPI.IntegrationTests.csproj`. The integration-tests project gets `ProjectReference` to the package too (closes code-review H1 + H2 simultaneously)
32. [ ] Delete `AZOA.WebAPI/Core/SurrealDb/Query/` directory entirely (the types now live in the package; AZOA imports via `using Azoa.SurrealDb.Client;`)
...
34. [ ] Delete `scripts/surrealdb/check-sdk-pin.ps1`, the `<SurrealDbNetPinnedVersion>` property + `<VerifySurrealSdkPin>` MSBuild target in `AZOA.WebAPI.csproj`, and `tests/AZOA.WebAPI.Tests/Core/SurrealDbSdkPinTests.cs`. Replaced by `Directory.Build.props` `<AzoaSurrealDbVersion>` (step 4)
...
36. [ ] Run `azoa-surreal generate Persistence/SurrealDb/Schemas/source/*.mermaid` and commit the regenerated `.surql` files alongside; assert byte-similar (modulo stylistic normalization) to wave-1 output; CI gate that source and generated stay in sync
37. [ ] Update `tests/AZOA.WebAPI.IntegrationTests/IntegrationTestBase.cs` and `Factories/AZOATestWebApplicationFactory.cs` to use the new client (`IServiceCollection.AddAzoaSurrealDb(...)`) instead of `SurrealDb.Net`'s `ISurrealDbClient`. Remove direct `HttpClient` SurrealDB probes
38. [ ] Update `scripts/passoff-surrealdb-wave1.ps1` section 2 to drop the `[G4 SDK-PIN OK]` literal assertion (the target no longer exists); replace with assert that `Azoa.SurrealDb.Client` is referenced at the version pinned in `Directory.Build.props`. Update section 3 (drift negative test) to fake-bump the `AzoaSurrealDbVersion` property instead. Update section 7 to reference the new analyzer package path
...
54. [ ] Begin the 90-day internal-soak window before considering public publish. Document decision criteria in `packages/PUBLISH-CRITERIA.md`: (a) zero LIVE-event-loss incidents, (b) zero schema-runner-corruption incidents, (c) zero blocking bugs reported by any internal consumer, (d) at least one second internal consumer beyond AZOA, (e) API surface unchanged for ≥30 consecutive days
...
- Generator round-trip: every `.mermaid` source regenerates byte-identical `.surql` on `azoa-surreal generate`
