...
| [surrealql-toolkit](tracks/surrealql-toolkit/spec.md) | `[ ]` | **Tier 4 — strategic ADR + umbrella program.** "Prisma for SurrealQL with first-class graph semantics." Names the constituent tracks (data-backfill-migrations, surrealql-drift-detection, surrealql-studio, surrealql-db-pull, surrealql-toolkit-packaging) + sequencing waves + seven shared principles. Foundation (Wave 0) already shipped via surrealdb-client-package + surrealdb-schema-source-gen + aggregate slice emitter. |
| [data-backfill-migrations](tracks/data-backfill-migrations/spec.md) | `[ ]` | **Tier 2 — surrealql-toolkit Wave 1.** First-class data-backfill primitive (`oasis-surreal backfill` CLI + `IBackfill` modules + `data_migration` ledger). F6 FK rewrite is the first real consumer (rewrite FK string columns as `record<table>` after RUNBOOK §4.3 Phase C). |
| [surrealql-drift-detection](tracks/surrealql-drift-detection/spec.md) | `[ ]` | **Tier 3 — surrealql-toolkit Wave 2.** `oasis-surreal drift` — diff deployed namespace vs local `.surql` sources; produce human-readable + JSON drift report. Mirrors Prisma `migrate diff`. |
| [surrealql-db-pull](tracks/surrealql-db-pull/spec.md) | `[ ]` | **Tier 3 — surrealql-toolkit Wave 2.** `oasis-surreal db pull` — reverse-engineer `.mermaid` sources from a running namespace. Closes the loop for users adopting the toolkit with an existing DB. Mirrors Prisma `db pull`. |
| [surrealql-studio](tracks/surrealql-studio/spec.md) | `[ ]` | **Tier 3 — surrealql-toolkit Wave 3.** Read-only browse + typed query + EXPLAIN UI at `/studio`. Avatar-scoped via the `mcp-surface` auth pattern. Graph-native traversal as the differentiator vs Prisma Studio. |
| [surrealql-toolkit-packaging](tracks/surrealql-toolkit-packaging/spec.md) | `[ ]` | **Tier 4 — surrealql-toolkit Wave 4.** Public NuGet packaging + docs site + sample repos + semver policy. Gated on Waves 1-3 + ≥3 months of OASIS dogfooding. |

## Shipped

20 tracks complete. One-line summaries — see each spec for detail.
