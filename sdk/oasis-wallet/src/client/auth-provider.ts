...
/**
 * OAuth-compatible auth provider using OASIS avatars as the identity source.
 *
 * Enables external apps to use OASIS as their authentication backend:
 * ```ts
 * const auth = oasis.createAuthProvider({ appName: "MyApp" });
 *
 * // Login flow
...
export class OasisAuthProvider {
  private readonly api: OasisApiClient;
  private readonly session: SessionManager;
  constructor(api: OasisApiClient, session: SessionManager, config?: AuthProviderConfig) {
    this.api = api;
