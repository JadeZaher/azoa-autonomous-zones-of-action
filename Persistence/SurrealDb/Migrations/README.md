...
The timestamp prefix is lexically sortable; the `oasis-surreal up` runner
...
`oasis-surreal up` is a no-op when no files changed.
...
2. **Force re-apply** — `oasis-surreal up --force` overwrites the
